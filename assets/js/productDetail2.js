...
function renderproducts(items) {
    ...
    productDeail2.forEach((element, index) => {
...
function addToCart(id) {
    if (getUserName) {
        let chooseItem = productDeail2.find((item) => item.id === id);
...
function incrementAmount(id) {
    productDeail2.forEach(() => {
        let item = productDeail2.find((item) => item.id === id); {
            if (item.require < 5) {
...
function AddToFav(id, clickedButton) {
    if (getUserName) { // assuming getUserName is a function
        let choosefavItem = productDeail2.find((item) => item.id === id);
        let additemfav = JSON.parse(localStorage.getItem("productFav")) || [];
