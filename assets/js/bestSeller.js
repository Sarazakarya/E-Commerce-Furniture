...
function Collection() {
    bestSelleritems.forEach((element) => {
...
function AddToFav(id, clickedButton) {
    if (getUserName) { // assuming getUserName is a function
        let choosefavItem = bestSelleritems.find((item) => item.id === id);
        let additemfav = JSON.parse(localStorage.getItem("productFav")) || [];
...
function AddToCart(id) {
    if (getUserName) {
        let chooseItem = bestSelleritems.find((item) => item.id === id);
