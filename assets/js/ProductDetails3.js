...
function renderproducts(items) {
    ...
    productDeail3.forEach((element, index) => {
...
function AddToFav(id, clickedButton) {
    if (getUserName) { // assuming getUserName is a function
        let choosefavItem = productDeail3.find((item) => item.id === id);
        let additemfav = JSON.parse(localStorage.getItem("productFav")) || [];
...
function addToCart(id) {
    if (getUserName) {
        let chooseItem = productDeail3.find((item) => item.id === id);
...
function incrementAmount(id) {
    productDeail3.forEach(() => {
        let item = productDeail3.find((item) => item.id === id); {
            if (item.require < 5) {
