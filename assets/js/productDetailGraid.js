...
function renderproducts(items) {
    ...
    ProductDetailGraid.forEach((element, index) => {
...
function addToCart(id) {
    if (getUserName) {
        let chooseItem = ProductDetailGraid.find((item) => item.id === id);
...
function incrementAmount(id) {
    ProductDetailGraid.forEach(() => {
        let item = ProductDetailGraid.find((item) => item.id === id); {
            if (item.require < 5) {
