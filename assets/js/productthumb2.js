...
function renderproducts(items) {
    ...
    productThumb2.forEach((element, index) => {
...
function AddToFav(id, clickedButton) {
    if (getUserName) { // assuming getUserName is a function
        let choosefavItem = productThumb2.find((item) => item.id === id);
        let additemfav = JSON.parse(localStorage.getItem("productFav")) || [];
...
function addToCart(id) {
    if (getUserName) {
        let chooseItem = productThumb2.find((item) => item.id === id);
...
function incrementAmount(id) {
    productThumb2.forEach(() => {
        let item = productThumb2.find((item) => item.id === id); {
            if (item.require < 5) {
