...
function renderproducts(items) {
    ...
    productThumb4.forEach((element, index) => {
...
function AddToFav(id, clickedButton) {
    if (getUserName) { // assuming getUserName is a function
        let choosefavItem = productThumb4.find((item) => item.id === id);
        let additemfav = JSON.parse(localStorage.getItem("productFav")) || [];
...
function addToCart(id) {
    if (getUserName) {
        let chooseItem = productThumb4.find((item) => item.id === id);
...
function incrementAmount(id) {
    productThumb4.forEach(() => {
        let item = productThumb4.find((item) => item.id === id); {
            if (item.require < 5) {
