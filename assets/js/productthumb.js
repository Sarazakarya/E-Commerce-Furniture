...
function renderproducts(items) {
    ...
    productThumb1.forEach((element, index) => {
        ...
        ProductDescription.innerHTML += `
    <h6>${element.titel}</h6>
                              <img src="../images/product-item/4.5.png" alt="">
                              ...
                              <div class="product-quntity">
                                  ...
                                  <div class="quntity">
                                      <div class="quntity-Amount">
                                          ...
                                          <p>${element.require}</p>
                                          <button class="btn-icon" onclick="inc(${element.id})"> <i class="fa-solid fa-plus"></i></button>
                                      </div>
...
function AddToFav(id, clickedButton) {
    if (getUserName) { // assuming getUserName is a function
        let choosefavItem = productThumb1.find((item) => item.id === id);
        let additemfav = JSON.parse(localStorage.getItem("productFav")) || [];
...
function addToCart(id) {
    if (getUserName) {
        let chooseItem = productThumb1.find((item) => item.id === id);
...
function incrementAmount(id) {
    productThumb1.forEach(() => {
        let item = productThumb1.find((item) => item.id === id); {
            if (item.require < 5) {
