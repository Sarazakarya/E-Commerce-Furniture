...
function renderproducts(items) {
    ...
    ProductDetailSlider.forEach((element, index) => {
...
function addToCart(id) {
    if (getUserName) {
        let chooseItem = ProductDetailSlider.find((item) => item.id === id);
...
function incrementAmount(id) {
    ProductDetailSlide.forEach(() => {
        let item = ProductDetailSlide.find((item) => item.id === id); {
            if (item.require < 5) {
...
function AddToFav(id, clickedButton) {
    if (getUserName) { // assuming getUserName is a function
        let choosefavItem = ProductDetailSlider.find((item) => item.id === id);
        let additemfav = JSON.parse(localStorage.getItem("productFav")) || [];
